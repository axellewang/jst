...
class MySnow:
    def __init__(self,dataID=None):
        ...
        self.countID = 0
        self.dataID = '10'    # 数据ID，这个自定义或是映射

    def get_id(self):
        # 时间差部分
        now = int(time.time())
        temp = now-self.start
        if len(str(temp)) < 9:  # 时间差不够9位的在前面补5
            length = len(str(temp))
            s = '5'* (9-length)
            temp = s + str(temp)
        if now == self.last:
    ...
    def get_gift(self):
        ...
        if len(countIDdata) < 2:  # 序列号2位
            ...
            s = str(2 - length)
            countIDdata = s + countIDdata
        id = (str(temp) + str(self.dataID) + countIDdata)[3:15]
        return id
